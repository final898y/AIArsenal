...
variables:
  ...
  - name: "course_name_or_source"
    ...
    required: false
summary: "將線上課程破碎、口語或雙語的原始素材轉化為 100% 完整保留知識、且自帶標準 YAML Metadata 與延伸概念解析的高品質繁體中文 Markdown 筆記。"
---

# AI Course Chapter Markdown Transformation Prompt

> **摘要與用途說明**
> 專門將線上課程平台（如 Coursera、DeepLearning.AI、edX 等）雜亂的網頁教材、講義與雙語字幕，轉換為具備標準知識管理 Metadata、嚴謹定義塊、概念架構圖、公式、程式碼、相關概念解析與自測題的高結構化 Markdown 學習筆記。
...
- **相關概念 (Related Concepts)**：列出 3~5 個與本章高度相關的延伸或相鄰專有名詞（附中英對照），並針對每個概念提供簡要說明與其和本章主題的關聯（如：上下游架構、底層演算法或互補技術）。**嚴禁捏造虛構的雙向連結或資料庫路徑（如 `[[02_Notes/...]]` 或 `[[03_Wiki/...]]`）**，保持乾淨易讀的清單格式，供使用者後續自行檢索與延伸。
...
- **[概念名稱 1 (Bilingual Term)]**：[簡要說明其核心定義，以及與本章主題的上下游、演算法依賴或互補關聯]
- **[概念名稱 2 (Bilingual Term)]**：[簡要說明其核心定義，以及與本章主題的上下游、演算法依賴或互補關聯]
- **[概念名稱 3 (Bilingual Term)]**：[簡要說明其核心定義，以及與本章主題的上下游、演算法依賴或互補關聯]
...
| :--- | :--- | :--- | :--- |
| `1.2.4` | 2026-09-25 | 調整相關概念為概念實體與關聯說明 | 移除虛構的 `[[02_Notes/...]]` 與 `[[03_Wiki/...]]` 內部路徑，改以中英概念名稱與簡明關聯說明呈現，避免網頁對話中產生幻覺連結 |
